import React from "react";
import { MdStarRate } from "react-icons/md";

const Reviews = () => {
  return (
    <div className="bg-blue-400 py-20">
      <div className="carousel w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        <div id="item2" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item3" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item4" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item5" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item6" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item7" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item8" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item9" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
        <div id="item10" className="carousel-item w-full">
        <div
          id="item1"
          className="carousel-item w-full flex-col items-center justify-center text-white gap-4"
        >
          <img
            src="https://img.daisyui.com/images/stock/photo-1625726411847-8cbb60cc71e6.webp"
            className="w-[100px] h-[100px] rounded-full border-2"
          />
          <h2>Md.Ashikuzzaman</h2>
          <div className="flex justify-center text-xl text-amber-400 gap-3 items-center">
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
            <MdStarRate />
          </div>
          <p>
            Its amazing for our country... It helps greatly to our common and <br /> village people..I hope it will continue its all service...
          </p>
        </div>
        </div>
      </div>
      <div className="flex w-full justify-center gap-2 py-6">
        <a href="#item1" className="btn btn-xs">
          1
        </a>
        <a href="#item2" className="btn btn-xs">
          2
        </a>
        <a href="#item3" className="btn btn-xs">
          3
        </a>
        <a href="#item4" className="btn btn-xs">
          4
        </a>
        <a href="#item5" className="btn btn-xs">
          5
        </a>
        <a href="#item6" className="btn btn-xs">
          6
        </a>
        <a href="#item7" className="btn btn-xs">
          7
        </a>
        <a href="#item8" className="btn btn-xs">
          8
        </a>
        <a href="#item9" className="btn btn-xs">
          9
        </a>
        <a href="#item10" className="btn btn-xs">
          10
        </a>
      </div>
    </div>
  );
};

export default Reviews;
